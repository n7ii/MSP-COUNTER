...
const ProfilePage = () => {
	...
	const mockUserData = {
		...
		bio: JSON.stringify([
			{
				type: 'paragraph',
				children: [{ text: 'Hello! I am John Doe, a software engineer.' }],
			},
		]),
	};
	...
	const formik = useFormik({
		...
		initialValues: {
			...
			twitter: mockUserData?.socialProfiles?.twitter,
			facebook: mockUserData?.socialProfiles?.facebook,
			instagram: mockUserData?.socialProfiles?.instagram,
			github: mockUserData?.socialProfiles?.github,
			twoFactorAuth: mockUserData?.twoFactorAuth,
			weeklyNewsletter: mockUserData?.newsletter?.weeklyNewsletter || false,
			lifecycleEmails: mockUserData?.newsletter?.lifecycleEmails || false,
			promotionalEmails: mockUserData?.newsletter?.promotionalEmails || false,
			productUpdates: mockUserData?.newsletter?.productUpdates || false,
			bio: (mockUserData?.bio && (JSON.parse(mockUserData.bio) as Descendant[])) || [],
			gender: 'Male',
			theme: 'dark',
			birth: '1987-12-21',
		},
							...
							<div className='col-span-12 flex flex-col gap-4 sm:col-span-8 md:col-span-10'>
								...
								{activeTab === TAB.CONNECTED && (
									<>
										...
										{/*			const key = i as keyof typeof userData.socialAuth; // Ensure correct indexing*/}
										{/*			const status = userData?.socialAuth[key] || undefined;*/}
										...
										{/*					<div className='flex flex-shrink-0 items-center gap-4'>*/}
										{/*						<Button*/}
										{/*							icon={status ? 'HeroTrash' : 'HeroCog8Tooth'}*/}
