...
import VolteyTransDetail from '@/pages/reports/voltey/components/VolteyTransDetail.tsx';
...
} from '@/pages/reports/redux/queries/volteyApiSlice.ts';
import {
	createColumnHelper,
	getCoreRowModel,
...
const VolteyReportPage = () => {
	...
	const { data: dashboardData, isFetching: isDashboardLoading } = useGetVolteyDashboardQuery(
		{
			dateStart: appliedFilters?.dateStart,
			dateEnd: appliedFilters?.dateEnd,
		},
	...
	const [fetchAllHistory] = useLazyGetVolteyHistoryQuery();
	...
	const columns = useMemo(
		() => [
			...
			columnHelper.accessor((row) => row.userDetail?.wlname, {
				id: 'wlname',
				header: 'ຊື່ກະເປົາ',
				cell: (info) => info.getValue() || '-',
			}),
			...
			columnHelper.display({
				...
				cell: (info) => {
					...
					let iccid = row?.iccId || row?.iccid || row?.ICCId;
					if (!iccid && row?.response) {
						try {
							const parsed =
								typeof row.response === 'string'
									? JSON.parse(row.response)
									: row.response;
							iccid = parsed?.iccid || parsed?.iccId;
						} catch {
							iccid = '';
						}
					}
					return <span className='whitespace-nowrap'>{iccid || '-'}</span>;
				},
			}),
			columnHelper.accessor('dataAmount', {
				header: 'ຂໍ້ມູນ',
				cell: (info) => info.getValue() || '-',
			}),
			...
			columnHelper.display({
				id: 'priceLak',
				header: 'ລາຄາ (LAK)',
				cell: (info) => formatMoney(info.row.original.priceLak, 'LAK'),
			}),
			columnHelper.display({
				id: 'priceUsd',
				header: 'ລາຄາ (USD)',
				cell: (info) => formatMoney(info.row.original.priceUsd, 'USD'),
			}),
			columnHelper.display({
				id: 'retailPriceLak',
				header: 'ລາຄາຂາຍ (LAK)',
				cell: (info) => formatMoney(info.row.original.retailPriceLak, 'LAK'),
			}),
			columnHelper.display({
				id: 'retailPriceUsd',
				header: 'ລາຄາຂາຍ (USD)',
				cell: (info) => formatMoney(info.row.original.retailPriceUsd, 'USD'),
			}),
			columnHelper.display({
				id: 'fee',
				header: 'ຄ່າທຳນຽມ',
			...
			columnHelper.accessor('transStatus', {
	...
	const exportAllDataToExcel = async () => {
		...
		try {
			...
			if (allData?.body?.content?.length) {
				await exportAllToExcel(allData.body.content, 'ລາຍງານ Voltey');
			} else {
	...
	return (
		<>
			<ModalProvider
				...
				title='ລາຍລະອຽດ Voltey'
				scrollBehavior='outside'
				size='5xl'>
				<div className='py-4'>
					{transData && <VolteyTransDetail transaction={transData} />}
				</div>
			</ModalProvider>

			{isLoading && <Loading />}

			<PageWrapper name='ລາຍງານ Voltey'>
				<Subheader>
					<SubheaderLeft>
						<FieldWrap>
				...
				<Container>
					<VolteyDashboard data={dashboardData?.body} />

					<Card className='h-full'>
						<CardHeader>
							<CardHeaderChild>
								<CardTitle>ລາຍການທຸລະກໍາ Voltey</CardTitle>
								<Badge
									variant='outline'
...
export default VolteyReportPage;
